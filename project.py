...
def editHobby(hobby_name):
    ...
    if creator.id != login_session['user_id']:
        flash("You cannot edit this Hobby. This Hobby belongs to %s" %
              creator.name)
        return redirect(url_for('showCatalog'))
    if request.method == 'POST':
        ...
        return render_template('edithobby.html', hobbies=editedHobby,
                               hobby=hobby)
...
def deleteHobby(hobby_name):
    ...
    user = getUserInfo(login_session['user_id'])
    if creator.id != login_session['user_id']:
        flash("You cannot delete this Hobby. This Hobby belongs to %s" %
              creator.name)
        return redirect(url_for('showCatalog'))
    if request.method == 'POST':
...
def allItemsJSON():
    ...
    hobby_dict = [h.serialize for h in hobbies]
    for h in range(len(hobby_dict)):
        items = [i.serialize for i in session.query(Items)
                 .filter_by(hobby_id=hobby_dict[h]["id"]).all()]
        if items:
...
def hobbiesJSON():
    hobbies = session.query(Hobby).all()
    return jsonify(hobbies=[c.serialize for h in hobbies])
...
def addItem():
    ...
    if request.method == 'POST':
        newItem = Items(
            ...
            hobby=session.query(Hobby).filter_by(
                name=request.form['hobby']).first(),
...
@app.route('/catalog/<path:hobby_name>/<path:item_name>/delete',
           methods=['GET', 'POST'])
@login_required
...
@app.route('/catalog/<path:hobby_name>/<path:item_name>/edit',
           methods=['GET', 'POST'])
@login_required
