...
def editHobby(hobby_name):
    editedHobby = session.query(Hobby).filter_by(name=hobby_name).one()
    hobby = session.query(Hobby).filter_by(name=hobby_name).one()
...
def deleteHobby(hobby_name):
    hobbyToDelete = session.query(Hobby).filter_by(name=hobby_name).one()
...
def addItem():
    ...
    if request.method == 'POST':
        newItem = Items(
            ...
            hobby=session.query(Hobby).filter_by(name=request.form['hobby']).one(),
